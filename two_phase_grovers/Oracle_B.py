...
def build_oracle_B(clause_list, label="OracleB"):
    ...
    V_ARK0_BACK = list(range(97, 113))  # same var IDs as ARK0 but computed backward
    _V_DEEP_set = set(V_DEEP)
    # DEEP = shift_rows(CT XOR K2) where K2 = W4:W5
    # After shift_rows (swap nibbles 1<->3 in the 16-bit state):
    #   DEEP[0..3]   = W4[0..3] XOR CT[0..3]       (nibble 0 unchanged)
    #   DEEP[4..7]   = W5[4..7] XOR CT[12..15]     (nibble 1 gets nibble-3 of CT^K2)
    #   DEEP[8..11]  = W5[0..3] XOR CT[8..11]      (nibble 2 unchanged)
    #   DEEP[12..15] = W4[4..7] XOR CT[4..7]       (nibble 3 gets nibble-1 of CT^K2)
    _DEEP_MAP = []  # (W_var_id, ct_bit_idx) for DEEP[0..15]
    for _i in range(16):
        if _i < 4:    _DEEP_MAP.append((V_W4[_i],    _i))
        elif _i < 8:  _DEEP_MAP.append((V_W5[_i],    _i + 8))   # W5[4..7], CT[12..15]
        elif _i < 12: _DEEP_MAP.append((V_W5[_i - 8], _i))      # W5[0..3], CT[8..11]
        else:         _DEEP_MAP.append((V_W4[_i - 8], _i - 8))  # W4[4..7], CT[4..7]
    # MC_INV: GF(2) inverse of the SAES MixColumns matrix (8x8, per column-pair)
    # Synthesized by Qiskit LinearFunction transpile (verified: MC * MC_INV = I)
    _MC_INV_CX = [(4,1),(4,2),(5,2),(5,3),(2,5),(2,3),(3,2),(6,0),(2,6),(7,1),
                  (2,1),(1,2),(2,1),(1,0),(0,1),(1,0),(1,7),(1,6),(2,4)]
    # MC (forward): synthesized CX sequence for MixColumns
    _MC_CX = [(4,2),(5,2),(5,3),(6,0),(7,4),(5,4),(6,5),(5,6),(6,5),(3,5),
              (4,6),(6,4),(2,4),(6,7),(6,1),(1,6),(0,6),(0,5),(1,7)]

    def _apply_inv_mix_columns(q_col):
        """Apply inv_mix_columns to 8-qubit column using raw CX gates from MC_INV."""
        for src, tgt in _MC_INV_CX:
            fqc.cx(q_col[src], q_col[tgt])

    def _apply_mix_columns(q_col):
        """Apply mix_columns to 8-qubit column using raw CX gates from MC_MAT."""
        for src, tgt in _MC_CX:
            fqc.cx(q_col[src], q_col[tgt])

    def compute_ark0_backward():
        # ARK0_back = inv_sub_bytes(inv_shift_rows(inv_mix_columns(STX)))
        # = backward path: CT → K2 → s1m → s1s → s1 → s0 = ARK0_backward
        # For True Key: ARK0_back == KEY XOR PT
        # For wrong keys: ARK0_back != KEY XOR PT → ARK0 light clauses violated
        ...
        # Step 1: inv_mix_columns on each column-pair (col0 = bits 0-7, col1 = bits 8-15)
        _apply_inv_mix_columns(_q_ark[0:8])
        _apply_inv_mix_columns(_q_ark[8:16])
        # Step 2: inv_shift_rows = shift_rows (it's its own inverse: swap nibble1<->nibble3)
        for i in range(4):
    ...
    def uncompute_ark0_backward():
        ...
        for i in reversed(range(4)):
            ...
            fqc.cx(_q_ark[12+i], _q_ark[4+i])
            fqc.cx(_q_ark[4+i], _q_ark[12+i])
        # Undo step 1: mix_columns (forward) to cancel inv_mix_columns
    ...
    def eval_clauses(clauses):
        for cl_tuple in clauses:
            ...
            for lit in clause:
                ...
                # ARK0: NO substitution in Oracle B! V_ARK0_BACK is physically computed
                # from the backward path. Wrong keys produce ARK0_back != KEY XOR PT,
                # which causes light ARK0 clause violations that filter them.
                if v in _V_DEEP_set:
                    idx = v - 177
    ...
    # ── Backward path: compute STZ from CT side ──────────────────────────────────
    # STZ = inv_sub_bytes(shift_rows(CT XOR K2))  [cipher-side backward variable]
    # Step 1: V_STZ = W4:W5 XOR CT  (K2 XOR CT = s1m in the SAES formulation)
    ...
    for i in range(4):
        ...
        fqc.cx(pool[allocations[V_STZ[4+i]]], pool[allocations[V_STZ[12+i]]])
    # Step 3: inv_sub_bytes in-place -> V_STZ = s2 (backward round-1 state)
    ...
    # ── Backward ARK0: inv_sub_bytes(shift_rows(inv_mix_columns(STX))) ───────────
    # This is the backward PT XOR K0. For wrong keys, this != KEY XOR PT,
    # causing ARK0 light clause violations that filter all Phase-1 wrong survivors.
    compute_ark0_backward()
    fqc.barrier()

    # Evaluate STZ/STX/STZ1 boundary clauses (all have allocated registers)
    eval_clauses(stage_clauses['STZ'])
    eval_clauses(stage_clauses['STX'])
    eval_clauses(stage_clauses['STZ1'])
    fqc.barrier()

    # Evaluate ARK0 light clauses (using backward ARK0 register)
    eval_clauses(stage_clauses['ARK0'])
    fqc.barrier()

    # Uncompute backward registers
    uncompute_ark0_backward()
    fqc.barrier()
    uncompute_stx()
    fqc.barrier()

    # SR light clauses are intentionally SKIPPED in Oracle_B:
    # They reference V_ARK0 vars (97-112) which in Oracle_B map to V_ARK0_BACK
    # (the backward-computed ARK0 register). Computing SR = SubBytes(KEY^PT) and then
    # checking SR clauses against ARK0_BACK would produce false violations even for
    # True Key. All 69 Phase-1 survivors have 0 SR light violations classically,
    # so skipping SR loses no filtering power.

    # KEY and DEEP clauses (DEEP substituted to W4/W5 XOR CT in eval_clauses)
